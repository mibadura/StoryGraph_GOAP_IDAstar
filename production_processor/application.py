...
# definicje
# definiowanie świata
world_name = 'world_DragonStory' #'world_DragonStory'  #  #'world_DragonStory'  #   #'world_RumcajsStory'
# definiowanie misji
...
diagram_file_path = find_file(json_path, f'{quest_names[0][0:12]}_diagram_{quest_names[0][13:]}.drawio')
...
if not diagram_file_path:
    print(f"Nie ma w projekcie diagramu projektowego o nazwie: „{quest_names[0][0:12]}_diagram_{quest_names[0][13:]}.drawio”. Sprawdź nazwę diagramu lub testuj bez kolorowania przejścia.")
    if input("Czy chcesz przejść test bez kolorowania?").lower() != "t":
        exit()
...
character_paths = looking_for_main_character(gameplay, world, name_or_id=character_name, failure_text="Kończymy zanim zaczęliśmy, przy inicjacji.")
