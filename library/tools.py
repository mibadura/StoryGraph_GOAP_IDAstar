...
def world_copy(old_one, new_one, sort_elements = True, preserve_id = False):
    # sort_elements = True
    if type(new_one) in (int, float, bool, str): # new_one, żeby nie wchodzić w destination, które w starym są obiektem
        ...
        if preserve_id:
            # new_one['Id'] = old_one.get('Id')

            if not new_one.get('Id'):
                new_one['Id'] = str(uuid.uuid4())
        else:
        ...
        if 'Connections' in new_one:
            for dest1, dest2 in zip(old_one['Connections'], new_one['Connections']):
                ...
                if preserve_id:
                    if not dest2['Destination'].get('Id'):
                        dest2['Destination']['Id'] = str(uuid.uuid4())
                    dest2['Destination'] = dest2['Destination'].get('Id')
                else:
        ...
        for old_k, new_k in zip(old_one, new_one):
            if old_k in ["Locations", "Characters", "Items", "Narration"]:
                world_copy(old_one[old_k], new_one[new_k], sort_elements, preserve_id)
        ...
        for i in range(len(old_lst)):
            world_copy(old_lst[i], new_lst[i], sort_elements, preserve_id)
        return new_one
...
def draw_production_tree(production_dict, missing = None, mission_name = None, directory_path='../visualisation/out_production_hierarchy_tree', title_list_for_bold = None):
    ...
    if missing:
        ...
        for t in missing:
            if t.split(" / ")[0] in [x.split(" / ")[0] for x in production_dict]:
                # zasobochłonne a powinno paść na etapie walidacji
                t_missing = t.split(" / ")[0] + " (difference in cut part of the title)"
            else:
                t_missing = t.split(" / ")[0]
            graph.node(t_missing, fillcolor='#F8CECC', color='red', style='filled', shape='box')
            graph.edge('missing', t_missing, color='red')
    ...
    for p, v in production_dict.items():
        ...
        if v["parent"] != 'missing':
            ...
            if v["prod"]["TitleGeneric"].split(" / ")[0] in [x.split(" / ")[0] for x in production_dict]:
                # zasobochłonne a powinno paść na etapie walidacji
                v_missing = v["prod"]["TitleGeneric"].split(" / ")[0] + " (difference in cut part of the title)"
            else:
                v_missing = v["prod"]["TitleGeneric"].split(" / ")[0]
            graph.edge(v_missing, p.split(" / ")[0])
...
def draw_production_tree_st(production_dict, missing = None, mission_name = None, directory_path='../visualisation/out_production_hierarchy_tree', title_list_for_bold = None):
    if mission_name is None:
        mission_name = f'missions_{datetime.now().strftime("%Y%m%d%H%M%S")}'
    node_attributes = {
        'shape': 'box',
    ...
    for p, v in production_dict.items():
        ...
        graph.node(p.split(" / ")[0], **node_attributes, fillcolor=fc, URL= f'/Production?title={p.split(" / ")[0]}')
        if v["parent"] != 'missing':
            if  v.get("hierarchy_mismatch"):
                if "ERROR" in v.get("hierarchy_mismatch"):
                    edge_attributes = { 'style': 'dashed', 'color': 'red', "fontcolor": 'red'}
...
def sheaf_description_dict(location):
    location_info = {
        "desc": f'Lokacja {location.get("Name")} {"ma pewne właściwości: " + str(location.get("Attributes")) if location.get("Attributes") else "nie ma żadnych własności"}.',
        "items": [],
        "characters": []
    }
    ...
    if location.get("Items") and len(location['Items']) > 0:
        for ch in location['Items']:
            location_info["items"].append({
                "name": ch.get("Name"),
                "attributes": ch.get("Attributes") or None,
            })
        ...
        for ch in location['Characters']:
            ...
            if ch.get("Items") and len(ch['Items']) > 0:
                for it in ch['Items']:
                    character["items"].append({
                        "name": it.get("Name"),
                        "attributes": it.get("Attributes") or None
                    })
            if ch.get("Characters") and len(ch['Characters']) > 0:
                for it in ch['Characters']:
            ...
            if ch.get("Narration") and len(ch['Narration']) > 0:
                for it in ch['Narration']:
                    if it.get('Attributes') and len(it['Attributes']) > 0:
                        if it.get('Attributes').get('Knowledge'):
                            character["knowledge"] = it.get("Attributes").get("Knowledge")
                for it in ch['Narration']:
                    if it.get('Attributes') and len(it['Attributes']) > 0:
                        if it.get('Attributes').get('Sound'):
                            character["sound"] = it.get("Attributes").get("Sound")

            location_info["characters"].append(character)
