...
class GraphVisualizer:
    ...
    def _visualise_process(self, json_dict_or_list, parent_key: str, parent_name: str, graph: BaseGraph,
                           emph_nodes_ids: list = None, comments: dict = None, world=False, draw_id = True, map_style=False) -> list:
        """
        ...
        if isinstance(json_dict_or_list, dict):
            ...
            for k, v in json_dict_or_list.items():
                if k in "LSide":
                    current_list.extend(self._visualise_process(v, 'root', 'root', graph, emph_nodes_ids, comments, world, draw_id, map_style))
                elif k in ("Id", "Name", "Attributes", "Connections", "Preconditions", "Instructions"):
                    current_dict[k] = v
                elif k in "Locations":
                    current_list.extend(self._visualise_process(v, k, 'root', graph, emph_nodes_ids, comments, world, draw_id, map_style))
                elif k in ("Characters", "Items", "Narration"):
                    current_list.extend(self._visualise_process(v, k, parent_name, graph, emph_nodes_ids, comments, world, draw_id, map_style))
            ...
            for i in range(len(lst)):
                ...
                # background_colors['Characters'] = 'red'

                node_attributes = {
                    'shape': 'box',
                    'style': 'filled,rounded',
                    'bgcolor': background_colors[parent_key],
                    # 'fillcolor': None,
                    'color': background_colors[parent_key],
                }

                if parent_key == 'Locations' and map_style:  #
                    # background_colors['Locations'] = None
                    node_attributes['style'] = ''
                    node_attributes['penwidth'] = '0'
    ...
    def visualise(self, what: Union[list, dict], title: str = None, description: str = None,
                  world: bool = False, emph_nodes_ids: list = None, emph_edges: list = None, comments: dict = None, draw_id = True, map_style=False) -> BaseGraph:
        """
        ...
        self._vertex_counter = 1
        node_list = self._visualise_process(what, 'root', '', graph, emph_nodes_ids, comments, world=world, draw_id = draw_id, map_style=map_style) # sprawdzić, czy root czy Root
