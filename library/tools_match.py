            ...
            # dodane po problemach z 07
                ...
                if len(intersection) == 0:
                    ...
                    try:
                        ...
                        error_log += "Coś poszło bardzo nie tak z usuwaniem niepasującego węzła świata."
                    return False
            # koniec wrzutki po problemach z 07
...
def make_automatic_moves(gameplay, world, loc, productions_to_match, decision_nr, visualise = False):
    test_mode = False
    red_nodes = []
    ...
    if red_nodes_new:
        ...
        exit()  # Added to exit anytime there is a modificaiton error
...
def list_all_avaialble_actions_mb(gameplay, world, world_source, main_location, productions_to_match, decision_nr, character=None, visualise = True, char_index = None, loc_index = None, npc = False):
    char_text = ''
    test_mode = False
    ...
    for nr in range(len(productions_to_match)):  # [14:]
        if len(todos) > nr - offset and productions_to_match[nr]['Title'] == todos[nr - offset]['Title']:
            ...
            if productions_to_match[nr][
                'Title'] == "Teleportation / Teleportacja":  # ta produkcja blokowana jest zawsze, więc nie musimy sprawdzać
                cover = 'BLOKADA1 '
            elif blockades1:
                for b in blockades1:
            ...
            print(
                f"{all_prod_number_text}{nr - offset:02d}. {cover}{warning_text}{productions_to_match[nr]['Title'].split(' / ')[0]} – ",
                end="")
            print(f"{len(todos[nr - offset]['Matches'])} wariantów", end="")
            if test_mode:
                print('(', end='')
...
def character_turn(gameplay, world, world_source, main_location, productions_to_match, decision_nr, character=None,
    ...
    red_nodes = apply_chosen_actions_mb(visualise, variant, production, next_variant_idx, character, decision_nr, gameplay, world,
                            todos, next_action_idx, generate_files=generate_files)
    return red_nodes


def apply_chosen_actions_mb(visualise, variant, production, chosen_variant, character, decision_nr, gameplay, world, todos, nr, generate_files=None):
    # generowanie stanu świata przed zastosowaniem produkcji.
    if visualise:
        red_nodes, red_edges, comments = get_reds(variant)
    ...
    if visualise:
        ...
        # draw_graph(world, d_title, d_desc, d_file, d_dir, red_nodes, red_edges, comments) #MB CHANGE

        d_title = f'Świat w oczekiwaniu na ruch gracza'
        d_desc = f'Pomiędzy kolejnymi produkcjami'
        d_file = f'{decision_nr:03d}c_world_between_moves'
        if generate_files:
            draw_graph(world, d_title, d_desc, d_file, d_dir) #MB CHANGE
...
def check_hierarchy(parent, child, character = None) -> Tuple[str, list]:
    ...
    if todos:
        for variant in todos[0]["Matches"]:
            ...
            for elem in variant:
                ...
                if elem[0].get("Attributes") and not elem[1].get("Attributes"):
                    blocking_keys["Attributes"] = elem[1].get("Attributes")
                elif  elem[0].get("Attributes") and elem[1].get("Attributes"):
    ...
    if instr_OK:
        ...
        return 'WARNING: Instruction mismatch', blockades
    else:
        return "ERROR: LS mismatch", blockades
