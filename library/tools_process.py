...
class CannotFindCharacter(Exception):
...
def operation_winning(ls: list, variant: List[tuple], instruction: dict, prod_vis_mode = False) -> List[int]:
    character = ls_to_world(ls[0]["Characters"][0], variant)
    if not character.get("Attributes"):
        character['Attributes'] = {}
    if not character['Attributes'].get("IsWinner") and not prod_vis_mode:  # TODO działa tylko dla pojedynczej misji, trzeba poprawić
        print("""
                                   (
        *                           )   *
                      )     *      (
...
def apply_instructions_to_world(production: dict, variant: list, world: Union[list, dict], prod_vis_mode = False):
    ...
    for instruction_number, instruction in enumerate(instructions):
        ...
        # operacje testowe
            ...
            modified_nodes.extend(operation_winning(ls, variant, instruction, prod_vis_mode)) ### MB CHANGE: Commented this out to never win, but rather play forever
            # pass ###MB CHANGE
...
def dict_from_variant_preserve_id(variant):
    variant_dicts = []
    for ls_node, w_node in variant:
        # ls_node = ctypes.cast(ls_node_id, ctypes.py_object).value
        # w_node = ctypes.cast(w_node_id, ctypes.py_object).value
...
def looking_for_main_character(gameplay, world, name_or_id=None, pointer=None, failure_text = '', zero_text = '', multi_text = ''):
    if name_or_id:
        character_paths = breadcrumb_pointer(world, name_or_id=name_or_id, layer="Characters")
    elif pointer:
        character_paths = breadcrumb_pointer(world, pointer=pointer)
        name_or_id = pointer.get("Name")
    if not character_paths:
        reason = f"Nie ma postaci {name_or_id} w świecie. {failure_text + zero_text}"
        print(reason)
        game_over(gameplay, reason)
    if len(character_paths) > 1:
        reason = f"Niejednoznaczne wskazanie postaci {name_or_id} w świecie. {failure_text + multi_text}"
        game_over(gameplay, reason)

    return character_paths


def looking_for_main_character_for_api(gameplay, world, name_or_id=None, pointer=None, failure_text = '', zero_text = '', multi_text = ''):
    if name_or_id:
        character_paths = breadcrumb_pointer(world, name_or_id=name_or_id, layer="Characters")
    elif pointer:
        character_paths = breadcrumb_pointer(world, pointer=pointer)
    ...
    if not character_paths:
        reason = f"Nie ma postaci {name_or_id} w świecie. {failure_text + zero_text}"
        raise CannotFindCharacter(reason)
